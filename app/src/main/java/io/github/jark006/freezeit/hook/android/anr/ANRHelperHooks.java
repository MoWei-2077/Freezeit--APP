package io.github.jark006.freezeit.hook.android.anr;
...
public class ANRHelperHooks {
    ...
    public ANRHelperHooks(ClassLoader classLoader, Config config) {
        try {
                    ...
                    if (processRecordIndex == null) { // 没找到进程记录就找Anr记录
                            ...
                            XposedBridge.hookMethod(method, new AbstractMethodHook() {
                                ...
                                protected void beforeMethod(MethodHookParam param) {
                                    ...
                                    if (XpUtils.DEBUG_ANR)
                                        XpUtils.log("Freezeit[AnrHook]:", "跳过 ANR:" + XpUtils.getString(app, Enum.Field.processName));
                                }
                            });
                            success++;
                        }
                        ...
                        XposedBridge.hookMethod(method, new AbstractMethodHook() {
                            ...
                            protected void beforeMethod(MethodHookParam param) {
                                ...
                                param.setResult(null);
                                if (XpUtils.DEBUG_ANR)
                                    XpUtils.log("Freezeit[AnrHook]:", "跳过 ANR:" + XpUtils.getString(record, Enum.Field.processName));
                            }
                        });
                        success++;
                    }
            ...
            if (success > 0)
                XpUtils.log("Freezeit[Hook]:", "拦截应用无响应");
